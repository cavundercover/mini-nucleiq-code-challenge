...
- In the end I didn't really used __init__.py, I just left it empty because I realised it's not needed at this point because in the future this is not going to be a library, there is no reason to expose anything there, I leave it empty for convenience.
- One key decision in the project that I forgot to document earlier was the design of the analysis decision. It's more intuitive to think about it like: cell_count / total * 100 > threshold but after working at a bank I already know that it's way better to always work with ints, because of the IE3754, we should avoid. So it's a simple algebra equation that it's the same as cell_count * 100 > threshold * total where we use full int multiplication.